...
public class Exam_SetCollection {
	...
	// 자료구조의 특성에 따라서 List, Set, Map 선택하여 선택해야 함
		Set<Student> stdsSet = new HashSet<>();
		
		stdsSet.add(new Student("강동원",35,100));
		stdsSet.add(new Student("소지섭",35,90));
		stdsSet.add(new Student("정우성",85,70));
		printSize(stdsSet);
		stdsSet.add(new Student("강동원",35,100));
		printSize(stdsSet);
	
		Iterator<Student> it = stdsSet.iterator();
		while(it.hasNext()) {
	...
	public void strSetExample() {
		...
		// stdList.get(0) -> 값 출력(ArrayList는 순서로 가지고 출력)
